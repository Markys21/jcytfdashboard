...
<div class="row">
  <div class="login-box text-light">
    ...
    <div class="col-12 contaner-fluid mb-3 text-center text-light">
      <a class="text-decoration-none display-4 " href="index.php"><span class="text-warning fw-bolder">JCYTF</span>OFFICER</a>
...
$("#btnloginoff").click(function () {
  ...
  if (uname === '') {
    ...
    $.ajax({
      ...
      success: function (response) {
        if (response === 'Correct') {
          Swal.fire({ backdrop: false, title: 'Success', text: 'Login with OTP!', icon: 'success' })
            .then(() => {
              $("#otp").show();
              $("#btnloginoff").hide();
              countdown();
          ...
          Swal.fire({ backdrop: false, title: 'Error', text: response, icon: 'error' });
        }
      }
    });
  }
