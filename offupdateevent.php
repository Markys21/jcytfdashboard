...
//     $eventidupdated = $_GET['eventid'];
 ...
 //                  
    ...
    if($extFI != "" && $extSI == ""){
        ...
        userid=?, Speaker=?, position=?, attendees=?, venue=?, contact=?, email=?, 
        Description=?, date=?, time=?, created_at=?, updated_at=?,featuredimage=? where md5(id)=?');
        $stmt->execute([$eventname, $userid, $speaker, $position, $attendees, $venue, 
        ...
        $eventcreated, $eventupdated,  $featureImageName, $editeventid]);
            ...
            where md5(id)=?');
            $stmt->execute([$eventname, $userid, $speaker, $position, $attendees, $venue, $speakercontact, $speakeremail, $eventdesc, $eventdate, $eventtime, $eventcreated, $eventupdated,  $featureImageName, implode(',', $supportingImageNames), $editeventid]);
    }else if($extFI == "" && $extSI != ""){
        $stmt = $pdo->prepare('update tblevents set EventName=?, 
            userid=?, Speaker=?, position=?, attendees=?, venue=?, contact=?, email=?, 
            Description=?, date=?, time=?, created_at=?, updated_at=?,
             pictures=?
            where md5(id)=?');
            $stmt->execute([$eventname, $userid, $speaker, $position, $attendees, $venue, $speakercontact, $speakeremail, $eventdesc, $eventdate, $eventtime, $eventcreated, $eventupdated, implode(',', $supportingImageNames), $editeventid]);
    }else if($extFI == "" && $extSI == ""){
        $stmt = $pdo->prepare('update tblevents set EventName=?, 
        userid=?, Speaker=?, position=?, attendees=?, venue=?, contact=?, email=?, 
        Description=?, date=?, time=?, created_at=?, updated_at=?
        where md5(id)=?');
        $stmt->execute([$eventname, $userid, $speaker, $position, $attendees, $venue, $speakercontact, $speakeremail, $eventdesc, $eventdate, $eventtime, $eventcreated, $eventupdated, $editeventid]);
    }else{
        echo "Error";
    }
